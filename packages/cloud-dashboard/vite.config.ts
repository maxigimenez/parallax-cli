 ...
 * from `SENTINEL0_API_URL`; this plugin serves the same file in dev, so both
 ...
 * still puts the tag first, so `window.__SENTINEL0__` is set before the app
 * module runs.
 */
function envScript(): Plugin {
  return {
    name: 'sentinel0-env-script',
    configureServer(server) {
      server.middlewares.use('/env.js', (_request, response) => {
        const apiUrl = process.env.SENTINEL0_API_URL ?? 'http://127.0.0.1:8080'
        response.setHeader('Content-Type', 'application/javascript')
        response.end(`window.__SENTINEL0__=${JSON.stringify({ apiUrl })}\n`)
      })
    },
    transformIndexHtml() {
