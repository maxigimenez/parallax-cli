...
const THEME_CSS = readFileSync(join(ROOT, 'src/theme-nebula.css'), 'utf8')
 ...
 * So a nebula missing `--bits-primary-soft` renders one orange hover state in
 * an otherwise purple UI, and only a person looking at that state finds it.
 */
describe('the nebula theme', () => {
  it('is the theme the app actually asks for', () => {
    expect(APP_TSX).toContain('theme="nebula"')
    expect(THEME_CSS).toContain("[data-bits-theme='nebula']")
  })

  it('loads after the library, so its palette wins', () => {
    expect(MAIN_TSX.indexOf("'@16-bits-design/ui/styles.css'")).toBeLessThan(
      MAIN_TSX.indexOf("'./theme-nebula.css'")
    )
  })
  ...
  it('redefines every colour a built-in theme does', () => {
    ...
    const nebula = tokensIn(THEME_CSS, "[data-bits-theme='nebula']")

    expect(ocean.size).toBeGreaterThan(10)
    expect([...ocean].filter((token) => !nebula.has(token))).toEqual([])
  })
  ...
  it('sets --bits-ink, which ocean leaves inherited from ember', () => {
    ...
    // is wrong on purple, and inheriting it is exactly the silent failure above.
    expect(tokensIn(THEME_CSS, "[data-bits-theme='nebula']")).toContain('--bits-ink')
  })
  ...
  it('leaves geometry and type to the library', () => {
    ...
    const nebula = [...tokensIn(THEME_CSS, "[data-bits-theme='nebula']")]
    const structural = nebula.filter((token) =>
