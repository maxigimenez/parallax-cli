...
import { Bot, Github, Hash, Settings } from 'lucide-react'
  ...
  return (
    <aside className="h-full w-[340px] shrink-0 border-l border-zinc-800 bg-[#080808]">
      ...
      {activeView === 'tasks' ? (
        ...
        <div className="h-[calc(100%-124px)] overflow-y-auto p-4 space-y-5">
          <div>
          ...
          {config?.slack && (
            <div>
              ...
              <div className="rounded border border-zinc-800 bg-zinc-950 px-3 py-2 text-xs">
                <p className="text-zinc-500">Channel</p>
                <p className="mt-0.5 text-zinc-200">{config.slack.channel}</p>
              </div>
            </div>
          )}
        </div>
