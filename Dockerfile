...
RUN pnpm install --frozen-lockfile --prod --filter @parallax/cloud...

# ── build ──────────────────────────────────────────────────────────────────
FROM base AS build
RUN pnpm install --frozen-lockfile --filter @parallax/cloud...

COPY tsconfig.base.json ./
COPY packages/common packages/common
COPY packages/cloud packages/cloud

RUN pnpm --filter @parallax/common build && pnpm --filter @parallax/cloud build
...
COPY --from=prod-deps /app/packages/cloud/node_modules ./packages/cloud/node_modules
